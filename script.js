...
//     } else {
//         console.log('Error');
//         i--; // если условие не выполнилось мы возвращаемся на 1 итерацию назад и еще раз задаем вопросы пользователю
//     }
// }
// 
// 
...
// ! Function declaration создается до начала выполнения скриптаб можно вызвать перед обьявлением
// ! Function expression создается только тогда когда доходит поток кодаб можно вызвать только после обьявления 
...
const calc = (a, b) => { return a + b }; // если функция содержит один аргумент или функция в одну строчку, то скобки можно опускать const calc = (a, b) => a + b ;
